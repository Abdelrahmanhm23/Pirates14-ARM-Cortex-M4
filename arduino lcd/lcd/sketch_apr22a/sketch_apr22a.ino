...
void setup(void)
{
  ...
  LcdString("VOLTAGE");    // the desired char.
  Nokia5110_SetCursor(0,1);
   LcdString("ELosaz Omar");
   Nokia5110_SetCursor(0,2);
   LcdString("Pirates");
...
{
  delay(100);
 float voltage = analogRead(A2) * 5.0 / 1024;
  LcdXY (50,0);     // position of the "voltage" value on screen
  LcdString((dtostrf(voltage,5,0,string)));  //how the "voltage"value to be displayed  
  int omar= 50;
  LcdXY (50,1);
  LcdString((dtostrf(omar,5,0,string)));
    int pirates= 2017;
  LcdXY (50,2);
  LcdString((dtostrf(pirates,5,0,string)));
     int z= 23;
  LcdXY (50,3);
  LcdString((dtostrf(z,5,0,string)));
}
