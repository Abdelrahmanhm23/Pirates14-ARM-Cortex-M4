////////////////////////////////////////////////
/*
This file is used to set a controller (PID) for each motor of the lifters
Base on system identification on simulink to get transfer function to calculate desired response
Includes:
         - RPM function for each motor
				 - Reveloution for each motor 
...
////////Motor lifter 1////////
float kp_ML1=1.43596 , ki_ML1=0.11253, kd_ML1=-1.523;
...
////////Motor lifter 2////////
float kp_ML2=1.43596 , ki_ML2=0.11253, kd_ML2=-1.523;
...
////////Motor lifter 3////////
float kp_ML3=1.43596 , ki_ML3=0.11253, kd_ML3=-1.523;                  //  1.43596  ,   0.11253, -1.523
...
PID_value_ML1=	constrain(PID_value_ML1 , 10,15950 );
PID_value_ML1=	map(PID_value_ML1 , 0 , 15950 , 15950 , 10);
...
void PID_ML1_REV(int ref_ML1)
{
	...
	PID_value_ML1 = (kp_ML1*P_ML1) + (ki_ML1*I_ML1) + (kd_ML1*D_ML1);
  PID_value_ML1=	constrain(PID_value_ML1 , 10,15950 );
PID_value_ML1=	map(PID_value_ML1 , 10 , 15950 , 15950 , 10);
  previous_error_ML1=error_ML1;	
	analogWrite(Motor1,PID_value_ML1);
/*	if(ReadData()==R2)
	{liftersStop();
...
PID_value_ML2=	constrain(PID_value_ML2 , 10,15950 );
PID_value_ML2=	map(PID_value_ML2 , 0 , 15950 , 15950 , 10);
...
void PID_ML2_REV(int ref_ML2)
{
	...
	PID_value_ML2 = (kp_ML2*P_ML2) + (ki_ML2*I_ML2) + (kd_ML2*D_ML2);
PID_value_ML2=	constrain(PID_value_ML2 , 10,15950 );
PID_value_ML2=	map(PID_value_ML2 , 10 , 15950 , 15950 , 10);
  previous_error_ML2=error_ML2;	
	analogWrite(Motor2,PID_value_ML2);
	/*if(ReadData()==R2)
	{liftersStop();
...
PID_value_ML3=	constrain(PID_value_ML3 , 10,15950 );
PID_value_ML3=	map(PID_value_ML3 , 0 , 15950 , 15950 , 10);
...
void PID_ML3_REV(int ref_ML3)
{
	...
	PID_value_ML3 = (kp_ML3*P_ML3) + (ki_ML3*I_ML3) + (kd_ML3*D_ML3);
PID_value_ML3=	constrain(PID_value_ML3 , 10,15950 );
PID_value_ML3=	map(PID_value_ML3 , 10 , 15950 , 15950 , 10);
  previous_error_ML3=error_ML3;	
	analogWrite(Motor3,PID_value_ML3);
	if(ReadData()==R2)
	{liftersStop();
...
void PID_MServe_RPM(int rpm_ref_MServe)
{
  ...
  PID_value_MServe=	map(PID_value_MServe , 0 , 255 , 15950 , 1000);
